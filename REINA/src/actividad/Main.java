...
//		recolocaR(3);
	...
	private static void recolocaR() {
		r21 = new Reina("R21");
		loopr21: for (int ii = 0; ii < tablero.length; ii++) {
			for (int jj = 0; jj < tablero[0].length; jj++) {
				if (!(r1.x == ii && r1.y == jj)
						&& (daJaque(ii, jj) /* && !(coinciden(ii, jj, r22) || coinciden(ii, jj, r23)) */)) {
					r21.x = ii;
					r21.y = jj;
					colocaR(r21);
					break loopr21;
		...
		r22 = new Reina("R22");
		loopr22: for (int ii = 0; ii < tablero.length; ii++) {
			for (int jj = 0; jj < tablero[0].length; jj++) {
				if (!(r1.x == ii && r1.y == jj) && ((daJaque(ii, jj)) && ((!coinciden(ii, jj,
						r21) /* || r1EstaEnElPutoMedio(ii, jj, r21) */) /* || coinciden(ii, jj, r23) */))) {
					r22.x = ii;
					r22.y = jj;
					colocaR(r22);
					break loopr22;
		...
		r23 = new Reina("R23");
		loopr23: for (int ii = 0; ii < tablero.length; ii++) {
			for (int jj = 0; jj < tablero[0].length; jj++) {
				if (!(r1.x == ii && r1.y == jj)
						&& ((daJaque(ii, jj) && !coinciden(ii, jj, r21) && !coinciden(ii, jj, r22)))) {
					r23.x = ii;
					r23.y = jj;
					colocaR(r23);
					break loopr23;
	...
	private static boolean r1EstaEnElPutoMedio(int ii, int jj, Reina r2x) {
		if (((r2x.x < r1.x && r1.x < ii) && r1.y == jj)) {
			...
			return true;
		} else if ((Math.abs(r2x.x - r1.x) == Math.abs(r2x.y - r1.y)) && (Math.abs(ii - r1.x) == Math.abs(jj - r1.y))
				&& (Math.abs(ii - r1.x) == Math.abs(jj - r1.y))) {
	...
	private static boolean coinciden(int ii, int jj, Reina r2x) {
		if (/* (r2x.x == ii && r2x.y == jj) || */r2x.x == ii || r2x.y == jj
				|| Math.abs(ii - r2x.x) == Math.abs(jj - r2x.y)) {
			return true;
		}
		return false;
	}
